...
<?php 
    $id = $_GET['id'];
    $sql = "SELECT * FROM `food_description` WHERE food_categ_id=$id";
    $result = mysqli_query($conn,$sql);
    while($row = mysqli_fetch_assoc($result)){
        $id = $row['food_id'];
  ...
  <select name="option" id="option" class="form-control">
    ...
    <option value="Yellow Apple">Yellow</option>
  </select>
  </div>
  <div class="form-group">
    <label for="exampleInputPassword1">Quantity</label>
    <input type="text" class="form-control" id="quantity" placeholder="1 kg">
  </div>
